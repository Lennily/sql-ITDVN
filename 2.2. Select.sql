...
WHERE CreditCardID = 10; -- В конструкции WHERE применена операция (=)
...
WHERE ExpYear = 2005 OR ExpYear = 2006; -- Логическая операция "ИЛИ".

-- Производим выборку данных из таблицы CreditCard,
...
-- Производим выборку всех данных из столбцов CardType и ExpYear, таблицы CreditCard,
...
-- Производим выборку данных из столбцов CardType таблицы CreditCard,
-- где значения ячеек столбца CardType соответствует шаблону 'ColonialVoice'
...
-- Производим выборку данных из столбцов CardType и ExpYear, таблицы CreditCard,
-- где значения ячеек столбца CardType соответствует шаблону 'Dis%'.
SELECT CardType, ExpYear FROM Sales.CreditCard
WHERE CardType LIKE 'Dis%'  -- Вывод всех карт, имя типа которых начинается на 'Dis',
							-- знак % обозначает произвольное количество символов после Dis.

SELECT CardType, ExpYear FROM Sales.CreditCard
WHERE CardType = 'Dis%'  -- Выборка пустая, потому что такого значения как 'Dis%' нет в таблице.

-- Производим выборку данных из столбцов CardType и ExpYear, таблицы CreditCard,
-- где значения ячеек столбца CardType соответствует шаблону 'Vis__'.
SELECT CardType, ExpYear FROM Sales.CreditCard
WHERE CardType LIKE 'Vis__'  -- _ - нижнее подчеркивание определяет любой один символ после 'Vis__'.


-- Производим выборку данных из столбцов CardType и ExpYear, таблицы CreditCard,
-- где значения ячеек столбца CardType соответствует шаблону 'Vis_'.
SELECT CardType, ExpYear FROM Sales.CreditCard
WHERE CardType LIKE 'Vis_'  -- почему выборка пустая?

-- Производим выборку данных из столбцов CardType и ExpYear, таблицы CreditCard,
-- где значения ячеек столбца CardType соответствует шаблону 'Vis_a'.
SELECT CardType, ExpYear FROM Sales.CreditCard
WHERE CardType LIKE 'Vis_a'  

-- Производим выборку данных из столбцов CardType и ExpYear, таблицы CreditCard,
-- где значения ячеек столбца CardType соответствует шаблону '%ist%'.
SELECT CardType, ExpYear FROM Sales.CreditCard
